...
public class SimpleByteDistributor implements ByteDistributor
{

    @Override
    public byte[] distributeBitsBy(byte[] originalArr, int qInByte)
    {
    ...
    @Override
    public byte[] collectBitsBy(byte[] originalArr, int qInByte)
    {
        if (qInByte > 8 || qInByte < 1)
            throw new IllegalArgumentException("invalid parameter value 'qInByte'");
